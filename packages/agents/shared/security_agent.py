...
Model:   claude-haiku-4-5 (fast structured analysis)
...
async def security_agent_node(state: AgentState) -> AgentState:
    ...
    log.info("[SECURITY_NODE] Running security analysis | venture=%s platforms=%s dept=%s",
             venture_id, platforms, department)
    log_agent_event(run_id, venture_id, "SECURITY_AGENT", "RUNNING",
                    "OWASP scan + posting schedule")
    ...
    log.info("[SECURITY_NODE] ✓ Clearance granted | platforms=%s score=%d", platforms, llm_score)
